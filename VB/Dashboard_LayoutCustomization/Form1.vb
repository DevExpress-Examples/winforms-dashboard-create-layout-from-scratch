...
Namespace Dashboard_LayoutCustomization
    ...
    Public Partial Class Form1
        ...
        Private Sub ChangeLayout(ByVal args As DashboardToolbarItemClickEventArgs)
            ...
            Dim item0 As DashboardLayoutItem = New DashboardLayoutItem(dashboard.Items(0), 35)
            Dim item1 As DashboardLayoutItem = New DashboardLayoutItem(dashboard.Items(1), 65)
            Dim item2 As DashboardLayoutItem = New DashboardLayoutItem(dashboard.Items(2), 20)
            ' Create a layout group that contains two layout items.
            Dim group1 As DashboardLayoutGroup = New DashboardLayoutGroup(DashboardLayoutGroupOrientation.Horizontal, 80, item0, item1)
            ' Create a root layout group that contains the previously created group and 
            ' another layout item. 
            Dim rootGroup As DashboardLayoutGroup = New DashboardLayoutGroup(DashboardLayoutGroupOrientation.Vertical, 1, group1, item2)
        ...
        Private Sub RepositionItems(ByVal args As DashboardToolbarItemClickEventArgs)
            ...
            If rootGroup IsNot Nothing AndAlso rootGroup.ChildNodes.Count > 1 Then rootGroup.ChildNodes(0).MoveRight(rootGroup.ChildNodes(1))
        End Sub
        ...
        Private Sub LoadLayoutFromFile(ByVal obj As DashboardToolbarItemClickEventArgs)
            If File.Exists("DashboardLayout.xml") Then dashboardViewer1.LoadDashboardLayout("DashboardLayout.xml")
        End Sub
        ...
        Private Sub DashboardViewer1_CustomizeDashboardTitle(ByVal sender As Object, ByVal e As CustomizeDashboardTitleEventArgs)
            ...
            If exportItem IsNot Nothing Then e.Items.Remove(exportItem)
            Dim btnChangeLayout As DashboardToolbarItem = New DashboardToolbarItem()
            btnChangeLayout.Caption = "Change Layout"
            btnChangeLayout.ClickAction = New Action(Of DashboardToolbarItemClickEventArgs)(AddressOf Me.ChangeLayout)
            e.Items.Add(btnChangeLayout)
            Dim btnRepositionItems As DashboardToolbarItem = New DashboardToolbarItem()
            ...
            Dim btnSaveLayout As DashboardToolbarItem = New DashboardToolbarItem()
            btnSaveLayout.Caption = "Save Layout to File"
            btnSaveLayout.ClickAction = New Action(Of DashboardToolbarItemClickEventArgs)(AddressOf Me.SaveLayoutToFile)
            e.Items.Add(btnSaveLayout)
            Dim btnLoadLayout As DashboardToolbarItem = New DashboardToolbarItem()
            btnLoadLayout.Caption = "Load Layout from File"
            btnLoadLayout.ClickAction = New Action(Of DashboardToolbarItemClickEventArgs)(AddressOf Me.LoadLayoutFromFile)
